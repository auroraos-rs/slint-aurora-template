...
$aurora_psdk mb2 --target AuroraOS-5.2.0.75-base-armv7hl build

# Удаляем артефакты сборки
rm ./{{org_domain}}.{{project-name}}
rm documentation.list

$aurora_psdk rpmsign-external sign -k $PSDK_DIR/../../certs/regular_key.pem -c $PSDK_DIR/../../certs/regular_cert.pem "$CURRENT_DIR/RPMS/{{org_domain}}.{{project-name}}-$PKG_VERSION-1.armv7hl.rpm"
$aurora_psdk rpm-validator -p regular "$CURRENT_DIR/RPMS/{{org_domain}}.{{project-name}}-$PKG_VERSION-1.armv7hl.rpm"
